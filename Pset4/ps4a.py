...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList:
        inWord = True
    #check if word(string) is in hand(dict)
    wordDict = getFrequencyDict(word)
    handCopy = hand.copy()
    try:
        for key in wordDict:
            #Raise an error if any of the keys don't exist
            handCopy[key] -= wordDict[key]
            #Raise an error if any of the values get lesser than 0
            if handCopy[key] < 0:
                raise
        inHand = True
    except:
        inHand = False
    return inHand and inWord
    #dang son, this is so mafan
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    if calculateHandlen(hand) == 0:
        ...
        print('\nGoodbye! Total score:',score, 'points.')
...
def playGame(wordList):
    ...
    """
    #Counts how many time game is played, if count is 1, return error for 'r'
    # count = 0
    # try:
        
    #     inp = input("Enter n to deal a new hand, r to replay the last hand, or e to end game: ")
    #     #if input is not valid
    #     while inp != 'n' or inp != 'r' or inp != 'e':
    #         print('Invalid command.')
    #         inp = input("Enter n to deal a new hand, r to replay the last hand, or e to end game: ")
    #     #if input is new game
    #     if inp == 'n':
    #         playHand(dealHand(HAND_SIZE), wordList, HAND_SIZE)
    #         count += 1
    #     #if input is replay last hand
    #     if inp == 'r':
    #         if count < 1:
    # While loop, only broke when e is entered
    while True:
        try:
            ...
            # if r, replay hand
                ...
                #if there is no previous hand, NameError will be raise.
                playHand(curHand, wordList, HAND_SIZE)
                
            # if n, deal hand
            elif inp == 'n':
                #hand var is stored, in case replay
                curHand = dealHand(HAND_SIZE)
                playHand(curHand, wordList, HAND_SIZE)
            # else (Wrong entry, reloop
            else:
                print('Invalid command.')
        except NameError:
